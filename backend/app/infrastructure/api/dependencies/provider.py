...
from app.infrastructure.database.repositories import (
    PostgresUserRepository, PostgresEventRepository, 
    PostgresSessionRepository, PostgresRegistrationRepository,
...
def get_session_registration_use_cases(session: Session = Depends(get_session)) -> SessionRegistrationUseCases:
...
def get_auth_controller(user_use_cases: UserUseCases = Depends(get_user_use_cases)) -> AuthController:
    return AuthController(user_use_cases)

def get_event_controller(event_use_cases: EventUseCases = Depends(get_event_use_cases)) -> EventController:
    return EventController(event_use_cases)

def get_session_controller(session_use_cases: SessionUseCases = Depends(get_session_use_cases)) -> SessionController:
    return SessionController(session_use_cases)

def get_registration_controller(registration_use_cases: RegistrationUseCases = Depends(get_registration_use_cases)) -> RegistrationController:
    return RegistrationController(registration_use_cases)

def get_session_registration_controller(use_cases: SessionRegistrationUseCases = Depends(get_session_registration_use_cases)) -> SessionRegistrationController:

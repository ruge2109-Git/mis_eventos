...
from app.infrastructure.api.dependencies.provider import get_registration_controller, RequireAuthenticated
...
@router.post(
    ...
    description="Registers the authenticated user's attendance to an event. Requires Authentication."
)
...
def get_user_registrations(user_id: int, controller: RegistrationController = Depends(get_registration_controller)):

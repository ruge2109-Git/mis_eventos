...
class SessionRegistrationRequest(BaseModel):
    session_id: int = Field(
        ..., description="The ID of the session to register for", examples=[1]
    )


class SessionRegistrationResponse(BaseModel):
    id: int = Field(
        ..., description="The unique system ID of the session registration log"
    )
...
@router.post(
    ...
    responses={
        404: {
            "model": ErrorResponse,
            "description": "Not Found: Event or Session does not exist.",
        },
        409: {
            "model": ErrorResponse,
            "description": (
                "Conflict: User already registered to this session, or overlapping session."
            ),
        },
    },
)
def register_to_session(
    ...
    controller: SessionRegistrationController = Depends(
        get_session_registration_controller
    ),
...
@router.delete(
    ...
    description=(
        "Deletes the session registration for the authenticated user."
    ),
    responses={
        404: {
            "model": ErrorResponse,
...
@router.get(
    ...
    description=(
        "Shows all sessions a particular user has registered for."
    ),
    responses={

...
class PostgresSessionRegistrationRepository(BaseRepository[SessionRegistrationModel], SessionRegistrationRepository):
    ...
    def get_by_user_and_session(self, user_id: int, session_id: int) -> Optional[DomainSessionRegistration]:
    ...
    def list_by_session(self, session_id: int) -> List[DomainSessionRegistration]:
        statement = select(SessionRegistrationModel).where(SessionRegistrationModel.session_id == session_id)

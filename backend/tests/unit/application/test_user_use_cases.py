...
class TestUserUseCases:
    ...
    @pytest.fixture
    def use_cases(self, mock_user_repo):
        return UserUseCases(user_repo=mock_user_repo)

    def test_register_user_success(self, use_cases, mock_user_repo):
    ...
    def test_authenticate_invalid_password(self, use_cases, mock_user_repo, mocker):
        user = User(
            email="test@test.com", 
            full_name="N", 
        ...
        mocker.patch("app.application.use_cases.user_use_cases.pwd_context.verify", return_value=False)
        
        with pytest.raises(AuthenticationError):
            use_cases.authenticate_user(email="test@test.com", password="wrong_password")

    def test_authenticate_success(self, use_cases, mock_user_repo, mocker):
        user = User(
            email="test@test.com", 
            full_name="N", 
            hashed_password="hashed_pwd", 
        ...
        mock_user_repo.get_by_email.return_value = user
        mocker.patch("app.application.use_cases.user_use_cases.pwd_context.verify", return_value=True)

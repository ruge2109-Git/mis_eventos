...
class TestSessionRegistrationAPI:
    ...
    def test_unregister_session_success(self, client, attendee_auth_headers, setup_data):
        client.post("/registrations/", json={"event_id": setup_data["event_id"]}, headers=attendee_auth_headers)
        client.post("/session-registrations/", json={"session_id": setup_data["session_id"]}, headers=attendee_auth_headers)
    ...
    def test_list_user_sessions(self, client, attendee_auth_headers, setup_data):
        reg = client.post("/registrations/", json={"event_id": setup_data["event_id"]}, headers=attendee_auth_headers)
        user_id = reg.json()["user_id"]
        client.post("/session-registrations/", json={"session_id": setup_data["session_id"]}, headers=attendee_auth_headers)

        response = client.get(
            f"/session-registrations/user/{user_id}",
            headers=attendee_auth_headers
        )

...
export class RegisterToSessionUseCase {
  private sessionRegistrationRepository = inject(SessionRegistrationRepository);
  private authStore = inject(AuthStore);

  execute(sessionId: number): Observable<SessionRegistration> {
    const userId = this.authStore.userId();
    if (userId == null) {
      throw new Error('User must be authenticated to register for a session');
    }
    return this.sessionRegistrationRepository.registerToSession(sessionId);
  }
}
